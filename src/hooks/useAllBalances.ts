...
export const useAllBalances = () => {
  ...
  const checkTokenBalances = useCallback(async (
    ...
    try {
      ...
      const walletBalance = formatBalance(walletBalanceResult as bigint, decimals);
      const depositedBalance = formatBalance(depositedBalanceResult as bigint, decimals);
      const lockedBalance = formatBalance(lockedBalanceResult as bigint, decimals);
      ...
      console.error(`Error checking balances for ${tokenSymbol} on ${network}:`, error);
  ...
  const fetchAllBalances = useCallback(async () => {
    ...
    try {
      ...
      const tokensWithBalances = allBalances.filter(balance => balance.hasAnyBalance);
