...
const PercentageButton = React.memo(({ 
  percentage, 
  isActive, 
  onClick 
}: { 
...
PercentageButton.displayName = 'PercentageButton';

// Memoized network selector component
...
const SimpleForm = React.memo(({ tradingPair }: SimpleFormProps): JSX.Element => {
  ...
  const percentageButtons = useMemo(() => 
    [10, 25, 50, 75, 100].map((percentage) => (
      <PercentageButton
        ...
        isActive={formState.percentageValue === percentage}
        onClick={() => handlePercentageClick(percentage)}
      />
    )),
    [formState.percentageValue, handlePercentageClick]
  );

  // Memoize the fee calculation
  const feeAmount = useMemo(() => {
    ...
    return formatDecimalConsistent(fee);
  }, [formState.amount]);

  // Memoize the button styling and text
  const buttonProps = useMemo(() => {
    ...
    return {
      className: isBaseChain ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700",
      text: isBaseChain ? "Sell" : "Buy"
    };
  }, [currentChainId, getCurrentChainConfig]);

  // Memoize the address display
              ...
              <section className="flex flex-col gap-2 flex-1">
                ...
                <span className="flex items-center gap-2">
                  <NetworkSelector
                    ...
                    getCurrentChainConfig={getCurrentChainConfig}
                  />
          ...
          <section className="bg-gray-800 rounded-lg p-3 sm:p-1">
            <article className="flex items-center gap-2 px-3 py-3 sm:py-2">
              ...
              <span className="text-white">
                Fee: {feeAmount} {tradingPair?.quoteSymbol || "TTK"}
              </span>
            </article>
          </section>
