  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (line.startsWith('# ')) {
      ...
      if (emojiMatch) {
        ...
        const result = (
          <h1 key={index} className="text-4xl font-bold text-center mb-8">
            <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">{headerText}</span>
          </h1>
        );
        processedLines.push(result);
        continue;
    ...
    if (line.startsWith('## ')) {
      const headerText = line.replace('## ', '');
      const result = (
        <h2 key={index} className="text-3xl font-bold mt-12 mb-6 text-gray-800 border-b border-gray-200 pb-2">
          {headerText}
        </h2>
      );
      processedLines.push(result);
    ...
    if (line.startsWith('### ')) {
      const headerText = line.replace('### ', '');
      const result = (
        <h3 key={index} className="text-2xl font-bold mt-8 mb-4 text-gray-700">
          {headerText}
        </h3>
      );
      processedLines.push(result);
    ...
    if (line.startsWith('1. ')) {
      ...
      if (listItems.length > 1) {
        ...
        // Return the ordered list with proper indentation
        const result = (
          <ol key={index} className="list-decimal ml-8 mb-4 space-y-2">
            {listItems.map((item, itemIndex) => (
              <li key={itemIndex} className="text-gray-600 leading-relaxed pl-2">
                {item.content}
              </li>
            ))}
          </ol>
        ...
        const result = (
          <ol key={index} className="list-decimal ml-8 mb-4">
            <li className="text-gray-600 leading-relaxed pl-2">
              {listItems[0].content}
            </li>
          </ol>
        );
    ...
    if (line.includes('[') && line.includes('](') && line.includes(')')) {
      ...
      if (linkMatch) {
        ...
        linkMatch.forEach((link, linkIndex) => {
          ...
          if (text && url) {
            ...
            if (splitParts[0]) {
              ...
              if (splitParts[0].includes('**')) {
                ...
                boldParts.forEach((boldPart, boldIndex) => {
                  if (boldIndex % 2 === 0) {
                    parts.push(<span key={`text-${index}-${linkIndex}-${boldIndex}`}>{boldPart}</span>);
                  } else {
                    parts.push(<strong key={`bold-${index}-${linkIndex}-${boldIndex}`} className="text-gray-900 font-semibold">{boldPart}</strong>);
                  }
                });
              } else {
            ...
            const linkElement = isExternal ? 
              <a key={`${url}-${index}-${linkIndex}`} href={url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-medium">{text}</a> :
              <a key={`${url}-${index}-${linkIndex}`} href={url} className="text-blue-600 hover:underline font-medium">{text}</a>;
        ...
        if (currentText) {
          ...
          if (currentText.includes('**')) {
            ...
            boldParts.forEach((boldPart, boldIndex) => {
              if (boldIndex % 2 === 0) {
                parts.push(<span key={`remaining-text-${index}-${boldIndex}`}>{boldPart}</span>);
              } else {
                parts.push(<strong key={`remaining-bold-${index}-${boldIndex}`} className="text-gray-900 font-semibold">{boldPart}</strong>);
              }
            });
          } else {
