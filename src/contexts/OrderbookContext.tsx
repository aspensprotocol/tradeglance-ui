...
export function OrderbookProvider({
  ...
  const parseOrderbookData = useCallback(
    (entries: OrderbookEntry[]): OrderbookEntry[] => {
      ...
      return entries.map(entry => {
        ...
        if (typeof price === 'string' && parseFloat(price) > 1000000000000000000) {
          console.warn("⚠️ OrderbookContext: Detected wei price, converting to decimal:", price);
        }
        if (typeof quantity === 'string' && parseFloat(quantity) > 1000000000000000000) {
          console.warn("⚠️ OrderbookContext: Detected wei quantity, converting to decimal:", quantity);
        }
  ...
  const comprehensiveLoggingData = useMemo(() => ({
    orderbookBids: rawData.orderbook?.bids?.map((b, i) => ({
      index: i,
      price: b.price,
      quantity: b.quantity,
    ...
    orderbookAsks: rawData.orderbook?.asks?.map((a, i) => ({
      index: i,
      price: a.price,
      quantity: a.quantity,
    ...
    openOrders: rawData.openOrders?.map((o, i) => ({
      index: i,
      price: o.price,
      quantity: o.quantity,
  ...
  useEffect(() => {
    console.log("🔄 OrderbookContext: rawData changed, processing:", loggingData);
    
    // COMPREHENSIVE DEBUGGING: Log all raw data details
    console.log("🔍 OrderbookContext: COMPREHENSIVE raw data logging:", comprehensiveLoggingData);
  ...
  }, [rawData, parsedOrderbook, parsedOpenOrders, loggingData, comprehensiveLoggingData]);
